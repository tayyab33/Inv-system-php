<?php 
$title = "sales_return"; include_once('layouts/header.php'); 
session_start();
...
include_once('../private/file_roots.php');
 ...
 if(request_post()){
  ...
  if(check_product_exist_to_update($productcheck)){
	...
	$updateproduct['returnstock'] = $_POST['returnstock'];
	$re = updata_data_product_stock('product', $updateproduct); 
   if($re == true){
      echo "<div>" ?> 
...
<nav class="navbar nav bg-dark">
    <h3 class="m-auto m-2 text-danger"> Sales Return Page </h3>
</nav>
...
<form method="POST" action="" onload="off">
 ...
 <div class="form-outline mb-4">
     <label class="form-label" for="returnstock" style="font-weight: bold; color: dark;">Return Stock</label>
	<input type="text" name="returnstock" id="returnstock" class="form-control" autocomplete="off">
  </div>
